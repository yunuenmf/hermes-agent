...
- **Blockers enter coordinator-review before getting lost.** New blocker/review-required signals, crash/timeout circuit-breakers, and active-PR respawn guards create a dedicated `coordinator_review` holder task for the board coordinator profile. Treat it as the executable review lane: classify the source task, solve trivial admin/routing issues directly, delegate non-trivial implementation to the right responsible/profile, create deployment/review clusters when needed, re-dispatch the source only when automated recovery is safe, and preserve a human blocker only for deployment approval, credentials/secrets, destructive action, new scope, or another concrete human decision.
