...
KANBAN_GUIDANCE = (
    ...
    "7. **Talk directly, not through contact tasks.** Internal profile-to-profile "
    "discussion belongs on direct command/internal-message channels (for example "
    "`hermes -p <profile> chat -q '<question>' --toolsets safe` or a structured "
    "message via `send_message`/`hermes send` when a private profile room exists). "
    "Do not create Kanban contact tasks merely to ask, notify, or ping another "
    "profile. Use Kanban for durable work items, dependencies, blockers, evidence, "
    "and post-discussion outcomes.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
