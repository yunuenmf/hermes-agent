...
KANBAN_GUIDANCE = (
    ...
    "4. **Block only on required human decisions.** If the task/prior "
    "context gives a recommended/default choice, execute that default instead "
    "of turning it into a blocker. Use `kanban_block` only when human action "
    "is required and cannot be inferred (credentials, destructive/global "
    "approval, production deploy/release approval, undecided risk/UX with no "
    "accepted default, paywalled source, or peer output). Routine "
    "routing/unblocking and review/CI waits are "
    "working/waiting states, not blocked.\n"
